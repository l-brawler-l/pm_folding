...
/** \brief Test class. */
class TsFoldBuilder_1_Test : public ::testing::Test {
public:
    //-----<Types>-----
...
public:
    ...
    static const char*  LOG_FILE_RTS_04_3;              // log04-3.sq3 from folding algorithm topic
...
}; // class TsFoldBuilder_1_Test

// ---<do not change followings>---
const char* TsFoldBuilder_1_Test::LOG_FILE_RTS_04 = 
    CSVLOG1_TEST_LOGS_BASE_DIR "logs/log04.sq3";

const char* TsFoldBuilder_1_Test::LOG_FILE_RTS_04_1 =
    CSVLOG1_TEST_LOGS_BASE_DIR "logs/log04-1.sq3";

const char* TsFoldBuilder_1_Test::LOG_FILE_RTS_04_3 =
    CSVLOG1_TEST_LOGS_BASE_DIR "logs/log04-3.sq3";

const char* TsFoldBuilder_1_Test::LOG_FILE_RTS_05 =
    CSVLOG1_TEST_LOGS_BASE_DIR "logs/log05.sq3";

const char* TsFoldBuilder_1_Test::LOG_FILE_RUBIN_1 =
...
TEST_F(TsFoldBuilder_1_Test, settings1)
{
    using namespace xi::ldopa::ts;

    TsFoldBuilder b(nullptr, nullptr, nullptr);              // это надо под контролем держать
...
TEST_F(TsFoldBuilder_1_Test, aSimpleMineSettings1)
{
    ...
    // билдер
    TsFoldBuilder bldr(&log, &fnc, &pool);
}

//------------------------------------------------------------------------------

// построение простого лога
TEST_F(TsFoldBuilder_1_Test, buildSimpleLog1)
{
    instantiateNeedfulAttributeMembers();
    ...
    // проверяем атрибуты
    EXPECT_EQ(5, ts->getAttributeNum());

    TS::State st_init = ts->getInitState();
    TS::Attribute aa = Tsa::ss("a"); //TS::Attribute::ss("a");
    TS::StateRes rst_a = ts->getState(pool[{ aa }]);
    EXPECT_TRUE(rst_a.second);
    TS::TransRes rt_a = ts->getTrans(st_init, rst_a.first, aa); 
    EXPECT_TRUE(rt_a.second);

    // векторы Париха
    TS::ParikhVectorRes rst_a_pv = ts->getParikhVector(rst_a.first);
    EXPECT_TRUE(rst_a_pv.second);
    EXPECT_EQ(1, rst_a_pv.first.GetAttrsCnt().size());

    EXPECT_EQ(1, ts->getStateAttrCnt(rst_a.first, Tsa::ss("a")));
    EXPECT_EQ(0, ts->getStateAttrCnt(rst_a.first, Tsa::ss("b")));

    TS::StateRes rst_abc = ts->getState(pool[{ Tsa::ss("a"), Tsa::ss("b"), Tsa::ss("c") }]);
    TS::ParikhVectorRes rst_abc_pv = ts->getParikhVector(rst_abc.first);

    EXPECT_TRUE(rst_abc.second);
    EXPECT_EQ(3, rst_abc_pv.first.GetAttrsCnt().size());

    EXPECT_EQ(1, ts->getStateAttrCnt(rst_abc.first, Tsa::ss("a")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_abc.first, Tsa::ss("b")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_abc.first, Tsa::ss("c")));

    // обычное состояние
    TS::StateRes rst_bdce = ts->getState(pool[
        { Tsa::ss("b"), Tsa::ss("d"), Tsa::ss("c"), Tsa::ss("e")}]);
    EXPECT_TRUE(rst_bdce.second);
    EXPECT_EQ(0, ts->getStateAttrCnt(rst_bdce.first, Tsa::ss("a")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_bdce.first, Tsa::ss("b")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_bdce.first, Tsa::ss("c")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_bdce.first, Tsa::ss("d")));
    EXPECT_EQ(1, ts->getStateAttrCnt(rst_bdce.first, Tsa::ss("e")));
}

//------------------------------------------------------------------------------

// построение дерева Париха
TEST_F(TsFoldBuilder_1_Test, buildParikhTrie)
{
    instantiateNeedfulAttributeMembers();
    ...
    // лог
    SQLiteLog log(LOG_FILE_RTS_04_3);     // log04_3.sq3
    log.setAutoLoadConfig(true);        // явно указываем, что надо загружать конфиг и SQL для него
    //log.getAutoLoadConfigQry() = "SELECT * FROM DefConfig";
    log.setAutoLoadConfigQry("SELECT * FROM DefConfig");
    log.open();
    ...
    EXPECT_EQ(18, pt.getStatesNum());
    EXPECT_EQ(17, pt.getTransitionsNum());
...
TEST_F(TsFoldBuilder_1_Test, buildAndReduceSimpleLog1)
{
    instantiateNeedfulAttributeMembers();
    ...
    // лог
    SQLiteLog log(LOG_FILE_RTS_04_3);
    log.setAutoLoadConfig(true);        // явно указываем, что надо загружать конфиг и SQL для него
    //log.getAutoLoadConfigQry() = "SELECT * FROM DefConfig";
    log.setAutoLoadConfigQry("SELECT * FROM DefConfig");
    log.open();
    ...
    // редуцируем
    FixedIntListStateIdsPool newPool;
    CondensedStateFunc newFnc(&log, &newPool,
        &(ts->getMapOfAttrsToIndexes()));
    CycleCondensedTsBuilder reducer(ts, &newFnc, &newPool);
    TS* rts = reducer.build(1);                         // оставляем 1-ограниченные регионы
...
// TEST_F(TsFoldBuilder_1_Test, reduceRebuiltSimpleLog1)
// {
//     instantiateNeedfulAttributeMembers();
...
// TEST_F(TsFoldBuilder_1_Test, calcPrecision1)
// {
//     instantiateNeedfulAttributeMembers();
...
// // построение простого лога
// TEST_F(TsFoldBuilder_1_Test, buildSimpleTsFromLog4_Vis1)
// {
//     instantiateNeedfulAttributeMembers();
...
//     // записыватель
//     EvLogTSWithParVecsDotWriter dw;
//     dw.write(TS_TEST_MODELS_BASE_DIR "ts/TsFoldBuilder_1_Test-buildSimpleTsFromLog4_Vis1.gv", *ts);
// }

// //------------------------------------------------------------------------------

// // построение простого лога
// TEST_F(TsFoldBuilder_1_Test, buildSimpleTsFromLog4_SingleAcptState1)
// {
//     instantiateNeedfulAttributeMembers();
...
//     // записыватель
//     EvLogTSWithParVecsDotWriter dw;
//     dw.write(TS_TEST_MODELS_BASE_DIR "ts/TsFoldBuilder_1_Test-buildSimpleTsFromLog4_SingleAcptState1.gv", *ts);
// }
